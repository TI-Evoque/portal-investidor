...
import { ArrowLeft, CalendarRange, Download, Eye, FileText, Filter, FolderOpen, Search } from 'lucide-react'
...
export function InvestorUnitDetailsPage() {
  ...
  async function handlePreview(file: PortalFile) {
    setDownloadingId(file.id)
    try {
      const response = await api.get(`/files/${file.id}/download`, { responseType: 'blob' })
                        ...
                        {monthGroup.files.map((file) => (
                          <div className="unit-file-row investor-detail-file-row" key={file.id}>
                            ...
                            <div className="unit-inline-actions">
                              ...
                              >
                                <Eye size={16} />
                                {downloadingId === file.id ? 'Abrindo...' : 'Visualizar PDF'}
                              </button>
                              <button
