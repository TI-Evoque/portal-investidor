...
export function FilesPage() {
  ...
  const uniqueMonths = useMemo(() => Array.from(new Set(files.map((f) => f.mes_referencia))).sort(), [files])
  ...
  const filteredFiles = files.filter((file) => {
    ...
    const monthMatch = selectedMonth === 'all' || file.mes_referencia === selectedMonth
    ...
    <div>
      ...
      <div className="filters-grid">
        <div className="filter-box">
          ...
          {selectedUnit !== 'all' ? <button onClick={() => { setSelectedUnit('all'); setCurrentPage(1) }} className="filter-clear">✕</button> : null}
        </div>
        ...
        <div className="filter-box">
          ...
          {selectedType !== 'all' ? <button onClick={() => { setSelectedType('all'); setCurrentPage(1) }} className="filter-clear">✕</button> : null}
        </div>
          ...
          <div className="table-card flat">
            {paginatedFiles.map((file) => (
              <div className="file-row files-page files-page-grid" key={file.id}>
                ...
                <span>{file.mes_referencia}/{file.ano_referencia}</span>
