...
def _has_real_value(value: str) -> bool:
    ...
    return bool(normalized) and normalized.lower() not in {"seu-valor", "change-me", "changeme"}
...
def send_password_changed_email(*, to_email: str, user_name: str, new_password: str) -> None:
    ...
    body = {
        "message": {
            "subject": "Portal do Investidor Evoque - Nova senha temporária",
            "body": {
                "contentType": "HTML",
                "content": f"""
                <div style='font-family:Arial,sans-serif;color:#2f2f2f;'>
                  <h2 style='color:#ff5a00;'>Nova senha temporária</h2>
                  <p>Olá, {user_name or 'investidor'}.</p>
                  <p>Uma nova senha temporária foi gerada para o seu acesso no Portal do Investidor Evoque.</p>
                  <p>Use esta senha para entrar:</p>
                  <p style='font-size:28px;font-weight:700;letter-spacing:6px;color:#ff5a00;margin:18px 0;'>{new_password}</p>
                  <p>Depois do login, você será levado automaticamente para a tela de cadastrar uma nova senha antes de acessar o portal.</p>
