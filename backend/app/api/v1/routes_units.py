...
from fastapi import APIRouter, Depends, Form, HTTPException, UploadFile, File as FastFile
...
def _photo_url(unit: Unit) -> str:
    if unit.foto_data:
        # Remova o settings.API_V1_PREFIX para evitar a duplicação no frontend
...
def _apply_unit_payload(unit: Unit, payload: UnitCreate | UnitUpdate) -> None:
    ...
    if foto_value is not None:
        if foto_value.startswith('data:image/'):
            unit.foto_data = _decode_data_url(foto_value)
            unit.foto_url = ''
        else:
            unit.foto_url = foto_value
...
async def upload_unit_photo(
    ...
    if not (foto.content_type or '').startswith('image/'):
        raise HTTPException(status_code=400, detail='Selecione uma imagem válida')
    content = await foto.read()
    if not content:
        raise HTTPException(status_code=400, detail='Imagem vazia')
    if len(content) > 5 * 1024 * 1024:
        raise HTTPException(status_code=400, detail='A imagem não pode ultrapassar 5MB')
...
def get_unit_photo(unit_id: int, db: Session = Depends(get_db)):
    # Removido o Depends(require_authorized_user) para permitir acesso público à imagem
    unit = db.query(Unit).filter(Unit.id == unit_id).first()
